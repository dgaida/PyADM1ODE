...
The [examples/two_stage_plant.py](../../examples/two_stage_plant.py) example demonstrates a complete two-stage biogas plant with mechanical components, energy integration, and comprehensive process monitoring.
...
- **Mechanical Components**: Pumps for material transport and mixers for process enhancement (as the ADM1 is implemented as a CSTR the mixer has no effect on the process, only needed to calculate its energy demand)
...
- **Gas Management**: Dedicated storage for each digester with automatic overflow protection
...
Components (10):
  ...
  - Substrate Feed Pump (mixer)
  - Digester Transfer Pump (mixer)
  - Hydrolysis Mixer (mixer)
  - Methanogenesis Mixer (mixer)
  - Main CHP Unit (chp)
  - Main CHP Unit_flare (storage)
...
Connections (8):
  ...
  - Main CHP Unit -> Main CHP Unit_flare (gas)
